  ...
  std::thread th(
      [](bedrock::network::Socket new_sock) {
        while (true) {
          ...
          if (read.status == bedrock::network::SocketErrorStatus::kDisconnect) {
            cout_mutex.lock();
            ...
            cout_mutex.lock();
            std::cout << "[Server]: Error: " << new_sock.GetErrorMessage()
                      << std::endl;
            cout_mutex.unlock();
            break;
          }
          ...
          cout_mutex.lock();
          std::cout << "[Server]: Echoed " << read.data.first.size() << "bytes "
                    << "to :"
...
int ClientProcess(std::uint16_t port) {
  ...
  cout_mutex.lock();
  std::cout << "[Client]: Client is now connected to: "
            << static_cast<std::string>(addr) << std::endl;
  cout_mutex.unlock();
